...
class cloudflare_api
{
    ...
    */
    public function update_dns_record($identifier,$dns_record_id,$type,$name,$content,$ttl,$prox = true){
         if (!in_array($type, self::$VALID_DNS_TYPES))
         return "incorrect dns type";
        if($type === 'SRV'){
    ...
    * @type : A, AAAA, CNAME, TXT, SRV, LOC, MX, NS, SPF
    */
    public function create_dns_record($identifier,$type,$name,$content,$ttl,$prox = true){
         if (!in_array($type, self::$VALID_DNS_TYPES))
         return "incorrect dns type";
        if($type === 'SRV'){
			//$content = "$weight, $priority, $target, $service, $proto, $port"
